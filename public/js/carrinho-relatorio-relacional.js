...
const filterTotal = precos.map((value, indexValue) => {
    if (productDivided[indexValue].id === indexValue) {
        if (shirtBasic.includes(productDivided[indexValue].nameProduct)) {
            ...
            productShirtBasicTotal.push(shirtBasic.length, productDivided[indexValue].nameProduct, value);
        }
        if (shirtPolo.includes(productDivided[indexValue].nameProduct)) {
            productDivided[indexValue].valueProduct = value;
            productShirtPoloTotal.push(shirtPolo.length, productDivided[indexValue].nameProduct, value);
        }
        if (shorts.includes(productDivided[indexValue].nameProduct)) {
            productDivided[indexValue].valueProduct = value;
            productShortsTotal.push(shorts.length, productDivided[indexValue].nameProduct, value);
        }
        if (jeans.includes(productDivided[indexValue].nameProduct)) {
            productDivided[indexValue].valueProduct = value;
            productJeansTotal.push(jeans.length, productDivided[indexValue].nameProduct, value);
        }
    }
});
...
const totalProducts = multiplyShirt + multiplyShirtPolo + multiplyShorts + multiplyjeans;
console.log('Produto:', productShirtBasicTotal[1], 'Quantidade:', productShirtBasicTotal[0], 'Valor Unitário', productShirtBasicTotal[2].toFixed(2).replace('.', ','), 'Valor Total:', multiplyShirt.toFixed(2).replace('.', ','));
console.log('Produto:', productShirtPoloTotal[1], 'Quantidade:', productShirtPoloTotal[0], 'Valor Unitário', productShirtPoloTotal[2].toFixed(2).replace('.', ','), 'Valor Total:', multiplyShirtPolo.toFixed(2).replace('.', ','));
console.log('Produto:', productShortsTotal[1], 'Quantidade:', productShortsTotal[0], 'Valor Unitário', productShortsTotal[2].toFixed(2).replace('.', ','), 'Valor Total:', multiplyShorts.toFixed(2).replace('.', ','));
console.log('Produto:', productJeansTotal[1], 'Quantidade:', productJeansTotal[0], 'Valor Unitário', productJeansTotal[2].toFixed(2).replace('.', ','), 'Valor Total:', multiplyjeans.toFixed(2).replace('.', ','));
