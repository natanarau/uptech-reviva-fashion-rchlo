...
const estoque = [
    ...
    },
    {
        ...
        preco: 79.9,
        categoria: 1,
        descricao: 'A Camiseta Manga Curta Free Fire Branco é produzida em fibra natural, sendo muito macia e confortável. Com modelagem regular, o diferencial da peça é a estampa temática do jogo Free Fire. As peças estampadas são sempre uma boa escolha na hora de compor um look mais despojado. Aposte!',
        tamanhos_disponiveis: ['P', 'M', 'G'],
    ...
    },
    {
        ...
        preco: 99.9,
        categoria: 1,
        descricao: 'O Suéter Feminino Curto Tricot Liso Verde Pool Basics by Riachuelo, confeccionado em tricot, ideal para os dias mais frios. Possui modelo fechado e mangas longas. Perfeito para os dias de temperatura baixa, use sobre regatas e combine com calças de fit ajustado e sapatilhas.',
        tamanhos_disponiveis: ['P', 'M', 'G'],
    ...
    },
    {
        ...
        preco: 69.9,
        categoria: 1,
        descricao: 'A Blusa Feminina Manga Longa Texturizada Branco AK by Riachuelo vai se tornar sua peça preferida! Confeccionada em malha texturizada, a peça é super versátil, agregando estilo e conforto ao seu closet!',
        tamanhos_disponiveis: ['P', 'M', 'G'],
    ...
    },
    {
        ...
        preco: 99.9,
        categoria: 2,
        descricao: 'Confeccionado em malha macia, o Maiô Tradicional Verão Azul Turquesa Dript by Riachuelo é uma ótima pedida para curtir os momentos de lazer na praia ou piscina. Com modelagem tradicional, apresenta estampa frontal e alças com regulagem. Combine com um saída de praia para os melhores dias de sol.',
        tamanhos_disponiveis: ['P', 'M', 'G'],
...
if (localStorage.length === 0) {
    ...
    estoque.map((value) => {
        // SE NÃO TIVER FAZ UM MAP DO ESTOQUE
        localStorage.setItem(value.id, JSON.stringify(value)); // E ARMAZENA OS DADOS NO LOCALSTORAGE
    });
    console.log('Produtos salvo no localStorage');
}
else {
...
[...addCart].map((value) => {
    value.addEventListener('click', () => {
        ...
        if (item.quantidade_disponivel <= 0) {
            ...
            const availableQuantityCart = JSON.parse(localStorage.getItem(value.id));
            availableQuantityCart.carrinho += 1;
            availableQuantityCart.total =
                +availableQuantityCart.preco * availableQuantityCart.carrinho;
            localStorage.setItem(value.id, JSON.stringify(availableQuantityCart));
            const alertCart = document.querySelector('.alert');
            alertCart.classList.add('alert-flex');
            setTimeout(() => {
                alertCart.classList.remove('alert-flex');
            }, 5000);
        }
    });
});
