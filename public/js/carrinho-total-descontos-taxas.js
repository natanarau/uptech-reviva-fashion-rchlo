...
const calculato = (acc, valueCurrent) => acc + valueCurrent;
const withDiscount = [];
...
const handle2part = valueDiscounts.map((value, index) => {
    if (!productsDiscount2part.includes(value) &&
...
const valueTotalShirt = valueDiscounts.reduce(calculato, 0) - +productsDiscount2part + partDiscount;
...
console.log(valueTotalShirt + totalValuejean + valueshort, 'Valor total a Pagar');
